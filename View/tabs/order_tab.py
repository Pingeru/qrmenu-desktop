...
class Order_Tab(QWidget):
    ...
    def _build_queue(self):
        ...
        toolbar.addWidget(make_badge("Business API", "accent"))
        toolbar.addWidget(make_label("Filter", "muted"))
        self.status_filter = QComboBox()
        self.status_filter.addItems(["All", "Pending", "Preparing", "Ready", "Cancelled"])
        self.status_filter.currentTextChanged.connect(self._refresh_table)
        toolbar.addWidget(self.status_filter)
        layout.addLayout(toolbar)

        self.table = QTableWidget(0, 6)
        self.table.setHorizontalHeaderLabels(["Order", "Created", "Items", "Total", "Status", "Customer"])
    ...
    def _build_detail_panel(self):
        ...
        self.delete_button = make_button("Delete Order", "danger")
        self.delete_button.setEnabled(False)
        self.delete_button.clicked.connect(self._delete_order)
        delete_actions.addWidget(self.delete_button)
    ...
    def _load_backend_orders(self, show_error=False):
        ...
        self.orders = [self._order_from_api(order) for order in api_orders]
        if selected_id and not self._order_by_id(selected_id):
            self.selected_order_id = None
        self._refresh_table()
        timestamp = QDateTime.currentDateTime().toString("HH:mm:ss")
        self.sync_label.setText(f"Last sync: {timestamp} - {len(self.orders)} orders from qrmenu-api")
    ...
    def _order_from_api(self, order):
        ...
        items = [self._item_label(item) for item in order.get("items", [])]
        return {
            "id": order_id,
    ...
    def _item_label(self, item):
        ...
        return f"{quantity} x {label} - $ {price:.2f}"
    ...
    def _refresh_table(self, *args):
        ...
        for order in filtered:
            ...
            self.table.setItem(row, 2, QTableWidgetItem(str(order["item_count"])))
    ...
    def _refresh_summary(self):
        counts = {status: 0 for status in ["Pending", "Preparing", "Ready", "Cancelled"]}
        for order in self.orders:
    ...
    def _delete_order(self):
        if not self.selected_order_id or not self.order_controller:
            self.detail_status.setText("Select an order before deleting.")
            return

        order = self._order_by_id(self.selected_order_id)
        order_name = order["display_id"] if order else self.selected_order_id[-8:]
        choice = QMessageBox.question(
            self,
            "Delete order",
            f"Delete order {order_name} from qrmenu-api?",
        ...
        try:
            self.order_controller.delete_order(self.selected_order_id)
        except ApiError as exc:
            QMessageBox.warning(self, "Order delete failed", str(exc))
            return

        deleted_name = order_name
        self.selected_order_id = None
        self._load_backend_orders(show_error=False)
        self.detail_status.setText(f"Deleted order {deleted_name} from backend.")
